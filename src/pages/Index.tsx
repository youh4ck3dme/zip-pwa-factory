...
const Index = () => {
  ...
  const handleGenerate = async (query: string) => {
    ...
    try {
      // Agency routing guard for dev-bypass-user: check if query matches agency keywords
      const isAgencyQuery = (q: string) => {
        const lower = q.toLowerCase();
        const keywords = ["barber", "salon", "spa", "restaurant", "cafe", "bistro", "diner", "agency", "business", "shop", "store", "boutique", "studio", "landing", "landing page", "pwa", "progressive web app", "premium", "professional", "online booking", "appointment", "service"];
      ...
      if (user?.id === "dev-bypass-user") {
        ...
        // If agency query, use Agency Landing PWA pipeline
        if (isAgencyQuery(trimmed)) {
          ...
          await db.pipelines.put({
            ...
            steps: [
              {
                id: `step-1-${suffix}`,
                name: `Generate Agency Spec`,
                type: "ai_generate",
                prompt: `Generate a premium PWA specification for a local business landing page. Input: ${trimmed}`,
                expectedOutput: "json",
              ...
              },
              {
                ...
                type: "ai_generate",
                prompt: `Create a compelling hero section with headline, subheadline, and CTA button for: ${trimmed}`,
                expectedOutput: "json",
                outputKey: "heroSection"
              },
              {
                id: `step-3-${suffix}`,
                name: `Create Services Section`,
                type: "ai_generate",
                prompt: `List 3-4 key services for this business: ${trimmed}`,
                expectedOutput: "json",
                outputKey: "services"
              },
              {
                id: `step-4-${suffix}`,
                name: `Create Booking CTA`,
                type: "ai_generate",
                prompt: `Create a prominent booking call-to-action with compelling copy for: ${trimmed}`,
                expectedOutput: "json",
                outputKey: "bookingCTA"
              },
              {
                id: `step-5-${suffix}`,
                name: `Export Agency PWA Package`,
                type: "export",
          ...
          // Non-agency query: use dev pipeline
          await db.pipelines.put({
            id: fakeId,
            ...
            steps: [
              ...
              },
              {
                ...
                prompt: `Determine the user's intent based on their request '${trimmed}'. Analyze possible interpretations (e.g., 'start a new project', 'explain the project', 'fix the project', or 'translate project'). Cross-reference with the repository context to infer the most likely intent.`
              },
              {
                id: "step-3",
